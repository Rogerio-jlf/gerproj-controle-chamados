...
const FiltersTabelaOsContext = createContext<FiltersContextProps | undefined>(
   undefined
);
...
export function FiltersTabelaOsProvider({
   children,
}: {
   ...
   return (
      <FiltersTabelaOsContext.Provider
         value={{
            filters,
            setFilters,
            clearFilters,
...
export function useFiltersTabelaOs() {
   const context = useContext(FiltersTabelaOsContext);
   if (context === undefined) {
      throw new Error(
         'useFiltersTabelaOs deve ser chamado dentro de um FiltersTabelaOsProvider'
      );
   }
   return context;
}
