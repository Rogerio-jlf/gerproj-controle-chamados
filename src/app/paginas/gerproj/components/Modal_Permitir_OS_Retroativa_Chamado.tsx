...
export const useBackdatedPermissions = () => {
   ...
   const makeApiCall = async (method: string, data?: any): Promise<any> => {
      ...
      const response = await fetch(
         '/api/tabelas/chamado/permitir-os-retroativa-chamado',
         options
      );
   ...
   const loadPermissionsInternal = useCallback(
      async (resourceId?: string, chamadoId?: string) => {
         setLoading(true);
         try {
            let url = '/api/tabelas/chamado/permitir-os-retroativa-chamado';
   ...
   const fetchChamadoAndResources = useCallback(async () => {
      ...
      try {
         ...
         const chamadoResponse = await fetch(
            `/api/tabelas/chamado?codChamado=${chamadoId}`,
            {
               headers: {
                  Authorization: `Bearer ${token}`,
                  'Content-Type': 'application/json',
...
export const canUseBackdatedAppointments = (
   ...
   try {
      ...
      xhr.open(
         'GET',
         `/api/tabelas/chamado/permitir-os-retroativa-chamado?resourceId=${resourceIdToCheck}&chamadoId=${chamadoId}`,
         false
      );
