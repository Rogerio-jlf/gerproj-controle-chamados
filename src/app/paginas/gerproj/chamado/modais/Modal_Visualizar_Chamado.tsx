'use client';

// COMPONENTS
import { TooltipCondicionalTabelaChamado } from '../Tooltip_Condicional.';

// TYPES
import { TabelaChamadoProps } from '../../../../../types/types';

// HELPERS
import { corrigirTextoCorrompido } from '../../../../../lib/corrigirTextoCorrompido';

// FORMATTERS
import {
   formatarCodNumber,
   getStylesStatus,
} from '../../../../../utils/formatters';

// ICONS
import { MdDescription } from 'react-icons/md';
import { IoClose, IoCall } from 'react-icons/io5';
import { FaUser, FaClock, FaTasks, FaInfo } from 'react-icons/fa';

// ================================================================================
// INTERFACES
// ================================================================================
interface ModalVisualizarChamadoProps {
   isOpen: boolean;
   onClose: () => void;
   chamado: TabelaChamadoProps | null;
}

// ================================================================================
// CONSTANTES
// ================================================================================
const EMPTY_VALUE = 'n/a' as const;

// ================================================================================
// COMPONENTE PRINCIPAL
// ================================================================================
export function ModalVisualizarChamado({
   isOpen,
   onClose,
   chamado,
}: ModalVisualizarChamadoProps) {
   // Função para fechar o modal
   const handleCloseModalVisualizarChamado = () => {
      onClose();
   };

   // Se o modal não estiver aberto ou o chamado for nulo, não renderiza nada
   if (!isOpen || !chamado) return null;

   // ================================================================================
   // RENDERIZAÇÃO
   // ================================================================================
   return (
      <div className="animate-in fade-in fixed inset-0 z-60 flex items-center justify-center p-4 duration-300">
         {/* ===== OVERLAY ===== */}
         <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
         {/* ========== */}

         <div className="animate-in slide-in-from-bottom-4 relative z-10 max-h-[100vh] w-full max-w-[80vw] overflow-hidden rounded-2xl border border-teal-900 bg-white/70 transition-all duration-500 ease-out">
            {/* ===== HEADER ===== */}
            <header className="relative flex items-center justify-between bg-teal-700 p-6 shadow-sm shadow-black">
               <div className="flex items-center justify-center gap-6">
                  <IoCall className="text-gray-300" size={72} />
                  <div className="flex flex-col">
                     <h1 className="text-4xl font-extrabold tracking-widest text-gray-300 select-none">
                        DETALHES DO CHAMADO
                     </h1>
                     <p className="text-xl font-extrabold tracking-widest text-gray-300 italic select-none">
                        CÓDIGO {formatarCodNumber(chamado.COD_CHAMADO)}
                     </p>
                  </div>
               </div>
               {/* ========== */}

               <button
                  onClick={handleCloseModalVisualizarChamado}
                  className="group cursor-pointer rounded-full bg-white/20 p-3 transition-all hover:scale-110 hover:rotate-180 hover:bg-red-500 active:scale-95"
               >
                  <IoClose
                     className="text-white group-hover:scale-110"
                     size={24}
                  />
               </button>
            </header>
            {/* ==================== */}

            {/* ===== CONTEÚDO ===== */}
            <main className="overflow-y-auto bg-gray-50 p-6">
               <div className="grid grid-cols-3 gap-6">
                  {/* ===== COLUNA DA ESQUERDA ===== */}
                  <div className="space-y-6">
                     {/* ===== CARD INFORMAÇÕES GERAIS ===== */}
                     <div className="overflow-hidden rounded-xl bg-white shadow-md shadow-black">
                        <div className="flex items-center gap-3 bg-gradient-to-r from-blue-600 to-blue-700 px-4 py-3 shadow-sm shadow-black">
                           {/* HEADER */}
                           <FaInfo className="text-white" size={24} />
                           <h2 className="text-lg font-extrabold tracking-widest text-white select-none">
                              INFORMAÇÕES GERAIS
                           </h2>
                        </div>
                        {/* ===== */}
                        <div className="space-y-3 p-4">
                           {/* COD_CHAMADO */}
                           <div className="flex items-center justify-between border-b border-slate-200 py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Chamado
                              </span>
                              <span className="black text-base font-bold tracking-widest text-black select-none">
                                 {formatarCodNumber(chamado.COD_CHAMADO)}
                              </span>
                           </div>
                           {/* ===== */}

                           {/* STATUS_CHAMADO */}
                           <div className="flex items-center justify-between border-b border-slate-200 py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Status
                              </span>
                              <span
                                 className={`rounded-md px-6 py-1 text-sm font-bold tracking-widest italic shadow-md shadow-black select-none ${getStylesStatus(
                                    chamado.STATUS_CHAMADO
                                 )}`}
                              >
                                 {chamado.STATUS_CHAMADO || EMPTY_VALUE}
                              </span>
                           </div>
                           {/* ===== */}

                           {/* PRIOR_CHAMADO */}
                           <div className="flex items-center justify-between border-b border-slate-200 py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Prioridade
                              </span>
                              <span className="text-base font-bold tracking-widest text-black italic select-none">
                                 {chamado.PRIOR_CHAMADO || EMPTY_VALUE}
                              </span>
                           </div>
                           {/* ===== */}

                           {/* NOME_CLASSIFICACAO */}
                           <div className="flex items-center justify-between gap-10 py-2">
                              <span className="w-60 flex-shrink-0 text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Classificação
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={
                                    chamado.NOME_CLASSIFICACAO || EMPTY_VALUE
                                 }
                              >
                                 <span className="flex-1 truncate text-right text-base font-bold tracking-widest text-black italic select-none">
                                    {chamado.NOME_CLASSIFICACAO || EMPTY_VALUE}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                        </div>
                     </div>
                     {/* ========== */}

                     {/* ===== CARD DADOS TEMPORAIS ===== */}
                     <div className="overflow-hidden rounded-xl bg-white shadow-md shadow-black">
                        {/* HEADER */}
                        <div className="flex items-center gap-3 bg-gradient-to-r from-blue-600 to-blue-700 px-4 py-3 shadow-sm shadow-black">
                           <FaClock className="text-white" size={24} />
                           <h2 className="text-lg font-extrabold tracking-widest text-white select-none">
                              DADOS TEMPORAIS
                           </h2>
                        </div>
                        {/* ===== */}
                        <div className="space-y-3 p-4">
                           {/* DATA_HORA_CHAMADO */}
                           <div className="flex items-center justify-between border-b border-slate-200 py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Data/Hora Abertura
                              </span>
                              <span className="text-base font-bold tracking-widest text-black italic select-none">
                                 {chamado.DATA_HORA_CHAMADO || EMPTY_VALUE} h
                              </span>
                           </div>
                           {/* ===== */}

                           {/* DTENVIO_CHAMADO */}
                           <div className="flex items-center justify-between border-b border-slate-200 py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Data/Hora Atribuição
                              </span>
                              <span className="text-base font-bold tracking-widest text-black italic select-none">
                                 {chamado.DTENVIO_CHAMADO || EMPTY_VALUE}
                              </span>
                           </div>
                           {/* ===== */}

                           {/* CONCLUSAO_CHAMADO */}
                           <div className="flex items-center justify-between py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Data Conclusão
                              </span>
                              <span className="text-base font-bold tracking-widest text-black italic select-none">
                                 {chamado.CONCLUSAO_CHAMADO || EMPTY_VALUE}
                              </span>
                           </div>
                        </div>
                     </div>
                  </div>
                  {/* ==================== */}

                  {/* ===== COLUNA DO MEIO ===== */}
                  <div className="space-y-6">
                     {/* ===== CARD CONSULTOR, CLIENTE ===== */}
                     <div className="overflow-hidden rounded-xl bg-white shadow-md shadow-black">
                        {/* HEADER */}
                        <div className="flex items-center gap-3 bg-gradient-to-r from-green-600 to-green-700 px-4 py-3 shadow-sm shadow-black">
                           <FaUser className="text-white" size={24} />
                           <h2 className="text-lg font-bold tracking-widest text-white select-none">
                              CONSULTOR E CLIENTE
                           </h2>
                        </div>
                        {/* ===== */}
                        <div className="space-y-3 p-4">
                           {/* NOME_RECURSO */}
                           <div className="flex items-center justify-between gap-10 border-b border-slate-200 py-2">
                              <span className="w-60 flex-shrink-0 text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Consultor
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={corrigirTextoCorrompido(
                                    chamado.NOME_RECURSO || EMPTY_VALUE
                                 )}
                              >
                                 <span className="flex-1 truncate text-right text-base font-bold tracking-widest text-black italic select-none">
                                    {corrigirTextoCorrompido(
                                       chamado.NOME_RECURSO?.split(' ')
                                          .slice(0, 2)
                                          .join(' ') || ''
                                    ) || EMPTY_VALUE}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                           {/* ===== */}

                           {/* NOME_CLIENTE */}
                           <div className="flex items-center justify-between gap-10 py-2">
                              <span className="w-60 flex-shrink-0 text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Cliente
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={chamado.NOME_CLIENTE || EMPTY_VALUE}
                              >
                                 <span className="flex-1 truncate text-right text-base font-bold tracking-widest text-black italic select-none">
                                    {chamado.NOME_CLIENTE?.split(' ')
                                       .slice(0, 2)
                                       .join(' ') || EMPTY_VALUE}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                           {/* ===== */}
                        </div>
                     </div>
                     {/* ========== */}

                     {/* ===== CARD CONSULTOR, CLIENTE ===== */}
                     <div className="overflow-hidden rounded-xl bg-white shadow-md shadow-black">
                        {/* HEADER */}
                        <div className="flex items-center gap-3 bg-gradient-to-r from-green-600 to-green-700 px-4 py-3 shadow-sm shadow-black">
                           <FaTasks className="text-white" size={24} />
                           <h2 className="text-lg font-extrabold tracking-widest text-white select-none">
                              TAREFA E PROJETO
                           </h2>
                        </div>
                        {/* ===== */}
                        <div className="space-y-3 p-4">
                           {/* TAREFA_COMPLETA */}
                           <div className="flex items-center justify-between gap-10 border-b border-slate-200 py-2">
                              <span className="w-60 flex-shrink-0 text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Tarefa
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={corrigirTextoCorrompido(
                                    chamado.TAREFA_COMPLETA || EMPTY_VALUE
                                 )}
                              >
                                 <span className="flex-1 truncate text-right text-base font-bold tracking-widest text-black italic select-none">
                                    {corrigirTextoCorrompido(
                                       chamado.TAREFA_COMPLETA || EMPTY_VALUE
                                    )}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                           {/* ===== */}

                           {/* PROJETO_COMPLETO */}
                           <div className="flex items-center justify-between gap-10 py-2">
                              <span className="w-60 flex-shrink-0 text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Projeto
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={corrigirTextoCorrompido(
                                    chamado.PROJETO_COMPLETO || EMPTY_VALUE
                                 )}
                              >
                                 <span className="flex-1 truncate text-right text-base font-bold tracking-widest text-black italic select-none">
                                    {corrigirTextoCorrompido(
                                       chamado.PROJETO_COMPLETO || EMPTY_VALUE
                                    )}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                           {/* ===== */}
                        </div>
                     </div>
                  </div>
                  {/* ==================== */}

                  {/* ===== COLUNA DA DIREITA ===== */}
                  <div className="space-y-6">
                     {/* ===== CARD OBSERVAÇÕES, OUTROS ===== */}
                     <div className="overflow-hidden rounded-xl bg-white shadow-md shadow-black">
                        {/* HEADER */}
                        <div className="flex items-center gap-3 bg-gradient-to-r from-amber-600 to-amber-700 px-4 py-3 shadow-sm shadow-black">
                           <MdDescription className="text-white" size={24} />
                           <h2 className="text-lg font-bold tracking-widest text-white select-none">
                              OBSERVAÇÕES E OUTROS
                           </h2>
                        </div>
                        {/* ===== */}
                        <div className="space-y-3 p-4">
                           {/* ASSUNTO_CHAMADO */}
                           <div className="flex items-center justify-between gap-10 border-b border-slate-200 py-2">
                              <span className="w-60 flex-shrink-0 text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Assunto
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={corrigirTextoCorrompido(
                                    chamado.ASSUNTO_CHAMADO || EMPTY_VALUE
                                 )}
                              >
                                 <span className="flex-1 truncate text-right text-base font-bold tracking-widest text-black italic select-none">
                                    {corrigirTextoCorrompido(
                                       chamado.ASSUNTO_CHAMADO || EMPTY_VALUE
                                    )}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                           {/* ===== */}

                           {/* EMAIL_CHAMADO */}
                           <div className="flex items-center justify-between gap-10 border-b border-slate-200 py-2">
                              <span className="w-60 flex-shrink-0 text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Email
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={chamado.EMAIL_CHAMADO || EMPTY_VALUE}
                              >
                                 <span className="flex-1 truncate text-right text-base font-bold tracking-widest text-black italic select-none">
                                    {chamado.EMAIL_CHAMADO || EMPTY_VALUE}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                           {/* ===== */}

                           {/* SOLICITACAO_CHAMADO */}
                           <div className="flex items-center justify-between gap-10 border-b border-slate-200 py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Solicitação
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={corrigirTextoCorrompido(
                                    chamado.SOLICITACAO_CHAMADO || EMPTY_VALUE
                                 )}
                              >
                                 <span className="text-base font-bold tracking-widest text-black italic select-none">
                                    {corrigirTextoCorrompido(
                                       chamado.SOLICITACAO_CHAMADO ||
                                          EMPTY_VALUE
                                    )}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                           {/* ===== */}

                           {/* SOLICITACAO2_CHAMADO */}
                           <div className="flex items-center justify-between gap-10 py-2">
                              <span className="text-sm font-semibold tracking-widest text-slate-600 select-none">
                                 Solicitação 2
                              </span>
                              <TooltipCondicionalTabelaChamado
                                 content={corrigirTextoCorrompido(
                                    chamado.SOLICITACAO2_CHAMADO || EMPTY_VALUE
                                 )}
                              >
                                 <span className="text-base font-bold tracking-widest text-black italic select-none">
                                    {corrigirTextoCorrompido(
                                       chamado.SOLICITACAO2_CHAMADO ||
                                          EMPTY_VALUE
                                    )}
                                 </span>
                              </TooltipCondicionalTabelaChamado>
                           </div>
                        </div>
                     </div>
                  </div>
                  {/* ==================== */}
               </div>
            </main>
         </div>
      </div>
   );
}
