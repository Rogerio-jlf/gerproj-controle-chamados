...
function useFilters() {
   ...
   const debouncedRESPCLI_PROJETO = useDebouncedValue(filters.RESPCLI_PROJETO);
   const debouncedNOME_RECURSO = useDebouncedValue(filters.NOME_RECURSO);
   const debouncedSTATUS_PROJETO = useDebouncedValue(filters.STATUS_PROJETO);

   const debouncedFilters = useMemo(
      () => ({
         PROJETO_COMPLETO: debouncedPROJETO_COMPLETO,
         NOME_CLIENTE: debouncedNOME_CLIENTE,
         RESPCLI_PROJETO: debouncedRESPCLI_PROJETO,
         NOME_RECURSO: debouncedNOME_RECURSO,
         STATUS_PROJETO: debouncedSTATUS_PROJETO,
      }),
      [
         debouncedPROJETO_COMPLETO,
         debouncedNOME_CLIENTE,
         debouncedRESPCLI_PROJETO,
         debouncedNOME_RECURSO,
         debouncedSTATUS_PROJETO,
      ]
   );
...
export function TabelaProjeto({ isOpen, onClose }: Props) {
   ...
   const queryParams = useMemo(() => {
      ...
      const filterMappings: Record<FilterKey, string> = {
         ...
         RESPCLI_PROJETO: 'filter_RESPCLI_PROJETO',
         NOME_RECURSO: 'filter_NOME_RECURSO',
         STATUS_PROJETO: 'filter_STATUS_PROJETO',
      };
   ...
   const handleOpenModalVisualizarProjeto = useCallback(
      (codProjeto: number) => {
         const projeto = data.find(p => p.COD_PROJETO === codProjeto);
         if (projeto) {
   ...
   if (isLoading)
      return (
         <IsLoading isLoading title="Aguarde... Buscando Projetos no sistema" />
      );
